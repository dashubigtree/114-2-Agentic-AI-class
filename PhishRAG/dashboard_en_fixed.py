...
def _render_knowledge_graph(graph_data: dict) -> None:
    ...
    with st.expander(
        ...
        # ── Statistics summary ──
        # Use actual list lengths to keep metrics consistent with tab counts.
        # LightRAG's processing_info counters reflect internal graph traversal
        # totals before deduplication and may differ from the returned lists.
        c1, c2, c3 = st.columns(3)
        c1.metric('Entities Retrieved', len(entities))
        c2.metric('Relationships Retrieved', len(relationships))
        c3.metric('Final Chunk Count', len(chunks))
